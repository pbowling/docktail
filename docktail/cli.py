...
# ---------------------------------------------------------------------------
# trim-relaxed  (called inside SLURM scripts after GFN-FF optimisation)
# ---------------------------------------------------------------------------

@main.command(name="trim-relaxed")
@click.option("--lig-dir", required=True,
              help="Per-ligand output directory (contains relax_complex/xtbopt.pdb).")
@click.option("--config", "-c", required=True, help="Path to YAML config file.")
def trim_relaxed_cmd(lig_dir, config):
    """Trim GFN-FF-relaxed structures ready for xTB single-point scoring.

    Reads ``relax_complex/xtbopt.pdb`` from LIG_DIR, applies the trimming
    settings from CONFIG, and writes ``sp_complex.pdb``, ``sp_protein.pdb``
    (and ``subsystem.pdb`` when oniom=true) into LIG_DIR.

    This command is automatically inserted into SLURM scripts generated by
    ``docktail prepare`` when ``relax=true`` and ``trim=true``.
    """
    from pathlib import Path as _Path
    cfg = load_config(config)
    try:
        trim_relaxed_structures(_Path(lig_dir), cfg)
    except Exception as exc:
        click.echo(f"Error: {exc}", err=True)
        sys.exit(1)
